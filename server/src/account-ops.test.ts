...
test('delete removes the account + its keys and whoami then 401s', async () => {
  ...
  await fetch(`${base}/xrpc/internal.bps.apiKey.create`, {
    method: 'POST', headers: { 'content-type': 'application/json', cookie: c }, body: JSON.stringify({ label: 'doomed' }),
  })
  ...
  const keys = await db.selectFrom('api_key').selectAll().where('did', '=', did).execute()
