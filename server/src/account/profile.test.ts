...
test('fetchProfile hits the public AppView getProfile and maps fields', async () => {
  ...
  globalThis.fetch = (async (url) => {
    ...
    return new Response(JSON.stringify({ did, handle: 'alice.test', displayName: 'Alice', avatar: 'https://cdn/x.jpg' }),
      { status: 200, headers: { 'content-type': 'application/json' } })
  }) as typeof fetch
  const p = await fetchProfile(APPVIEW, did)
  assert.ok(calledUrl.startsWith(`${APPVIEW}/xrpc/app.bsky.actor.getProfile?actor=`))
...
test('fetchProfile omits optional fields when absent', async () => {
  globalThis.fetch = (async () =>
    new Response(JSON.stringify({ did, handle: 'bob.test' }), { status: 200, headers: { 'content-type': 'application/json' } })
  ) as typeof fetch
  const p = await fetchProfile(APPVIEW, did)
