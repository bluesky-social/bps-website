...
export async function loadKeyset(config: AppConfig): Promise<JoseKey[]> {
  if (config.oauthPrivateKey) {
    const key = await JoseKey.fromImportable(config.oauthPrivateKey, config.oauthKeyId)
    return [key]
  }
  if (!config.devMode) {
    throw new Error('No BPS_OAUTH_PRIVATE_KEY set outside dev mode')
  }
  logger.warn('No BPS_OAUTH_PRIVATE_KEY — generating an EPHEMERAL ES256 key (dev only; sessions reset on restart)')
