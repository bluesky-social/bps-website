...
test('allows the configured origin with credentials', async () => {
  const res = await fetch(`${base}/x`, { headers: { origin: 'http://localhost:3000' } })
  assert.equal(res.headers.get('access-control-allow-origin'), 'http://localhost:3000')
  assert.equal(res.headers.get('access-control-allow-credentials'), 'true')
})

test('does not reflect a foreign origin', async () => {
  const res = await fetch(`${base}/x`, { headers: { origin: 'http://evil.example' } })
...
test('answers preflight OPTIONS with 204 and allow headers', async () => {
  const res = await fetch(`${base}/x`, { method: 'OPTIONS', headers: { origin: 'http://localhost:3000', 'access-control-request-method': 'POST' } })
