...
//
// Two deliberate differences, both in the widget's favor rather than the
// sample's: it passes the equivalent bare NSIDs instead of lexicon schemas, so a
// visitor doesn't download the whole Bluesky lexicon bundle, and it asks for
// kinds: ['commit'] to keep identity/account/sync events off the wire entirely.
// Neither changes what gets printed — isCreate() drops the other kinds anyway,
// which is why the sample doesn't need to name them.
const PROOF_CODE_LINES = [
  '<span class="tk-kw">import</span> <span class="tk-pun">{</span> <span class="tk-cls">Jetstream</span><span class="tk-pun">,</span> <span class="tk-fn">isCreate</span> <span class="tk-pun">}</span> <span class="tk-kw">from</span> <span class="tk-str">\'@bsky/jetstream\'</span><span class="tk-pun">;</span>',
  ...
  '<span class="tk-kw">const</span> <span class="tk-id">collections</span> <span class="tk-pun">=</span> <span class="tk-pun">[</span><span class="tk-id">app</span><span class="tk-pun">.</span><span class="tk-prop">bsky</span><span class="tk-pun">.</span><span class="tk-prop">graph</span><span class="tk-pun">.</span><span class="tk-prop">follow</span><span class="tk-pun">,</span> <span class="tk-id">app</span><span class="tk-pun">.</span><span class="tk-prop">bsky</span><span class="tk-pun">.</span><span class="tk-prop">feed</span><span class="tk-pun">.</span><span class="tk-prop">repost</span><span class="tk-pun">,</span> <span class="tk-id">app</span><span class="tk-pun">.</span><span class="tk-prop">bsky</span><span class="tk-pun">.</span><span class="tk-prop">feed</span><span class="tk-pun">.</span><span class="tk-prop">post</span><span class="tk-pun">];</span>',
  '',
  '<span class="tk-kw">for await</span> <span class="tk-pun">(</span><span class="tk-kw">const</span> <span class="tk-id">event</span> <span class="tk-kw">of</span> <span class="tk-id">jetstream</span><span class="tk-pun">.</span><span class="tk-fn">live</span><span class="tk-pun">({</span> <span class="tk-id">collections</span> <span class="tk-pun">})) {</span>',
    ...
    const start = () => {
      ...
      streamCreates({
        ...
        onCreate: ({ did, collection, record }) => {
          ...
          // above would. streamCreates() has already applied isCreate(), so
          // what's left per branch is the collection half of the sample's
          // `isCreate(event, nsid)`.
