...
This is the flow: read the collection off Jetstream, map each record
...
Coop's API isn't an atproto service, so this side is just a WebSocket (via the [Jetstream SDK](/docs/jetstream-sdk)) and an
authenticated `fetch`.

```typescript
import { Jetstream } from "bsky-jetstream-preview"; // npm install bsky-jetstream-preview
...
for await (const evt of jetstream.live({ collections: ["site.standard.document"] })) {
...
- **[Jetstream](/docs/jetstream)** — the live tail, filtering, and endpoints this
  builds on.
- **[Jetstream SDK](/docs/jetstream-sdk)** — the TypeScript client used for the
  stream above, in place of a hand-rolled WebSocket.
