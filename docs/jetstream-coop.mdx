...
- **A `URL` field rejects `at://` URIs.** Coop validates `URL` fields as real
  ...
  clickable. The permalink conversion is the only transform the bridge applies.
...
item types, and batch-`POST`s to Coop's
...
- **The [Report API](https://roostorg.github.io/coop/latest/api/report.html).**
  ...
  `/api/v1/report` to create a Job directly, using the same item type and
  `data` shape as ingest.
...
setup, separate from ingest: create a queue and a routing rule.)
...
The document has now made a full round trip: it left the network through
Jetstream, was reviewed in Coop, and a label went back onto it through your
labeler, where any client that subscribes to your labeler will see it.
