...
<Tabs groupId="sdk">
  <TabItem value="ts" label="TypeScript">
    This script below will create a simple post with just a text field — the `post` action fills in the `createdAt` timestamp for you.
  ...
  <TabItem value="go" label="Go">
    After logging in (see [Bluesky API](/docs/bluesky-api#create-a-session) for the `client`), build a `FeedPost` record and submit it with `RepoCreateRecord`.
    ...
    `resp.Uri` holds the `at://` URI of the post and `resp.Cid` the content-hash.
  </TabItem>
</Tabs>
...
<Tabs groupId="sdk">
  ...
  <TabItem value="go" label="Go">
    This snippet sets the `Text` and `Langs` values of a post to be Thai and English.

    ```go
    // an example with Thai and English (US) languages
    post := &appbsky.FeedPost{
      Text:      "สวัสดีชาวโลก!\nHello World!",
      Langs:     []string{"th", "en-US"},
      CreatedAt: time.Now().UTC().Format(time.RFC3339),
    }
    ```
  </TabItem>
...
<Tabs groupId="sdk">
  ...
  <TabItem value="go" label="Go">
    The Go SDK does not ship a high-level rich-text builder. Construct facets directly as `appbsky.RichtextFacet` values — each with a byte range (UTF-8 offsets into `post.Text`) and a feature (`RichtextFacet_Mention`, `RichtextFacet_Link`, or `RichtextFacet_Tag`) — and assign them to `post.Facets`. See [Links, mentions, and rich text](/docs/about-bluesky-content/post-richtext) for how the byte indexing works, and the [Go SDK](https://github.com/bluesky-social/indigo) for the struct definitions.
  </TabItem>
</Tabs>
  ...
  <TabItem value="go" label="Go">
    ...
    post := &appbsky.FeedPost{
      ...
      Reply: &appbsky.FeedPost_ReplyRef{
        Root:   &comatproto.RepoStrongRef{Uri: threadRootPost.Uri, Cid: threadRootPost.Cid},
        Parent: &comatproto.RepoStrongRef{Uri: postReplyingTo.Uri, Cid: postReplyingTo.Cid},
      },
    }
    ```
  </TabItem>
...
<Tabs groupId="sdk">
  <TabItem value="ts" label="TypeScript">
    ...
    const { body } = await client.uploadBlob(convertDataURIToUint8Array(image), {
      encoding: 'image/png',
    })
    await client.call(post, {
      text: 'I love my cat',
  ...
  <TabItem value="go" label="Go">
    Upload the image with `RepoUploadBlob`, then reference the returned blob in an `EmbedImages` embed:
    ...
    imgBytes, _ := os.ReadFile("my_cat.jpg")
    uploadResp, _ := comatproto.RepoUploadBlob(ctx, client, bytes.NewReader(imgBytes))
...
<Tabs groupId="sdk">
  <TabItem value="ts" label="TypeScript">
    ...
    const { body } = await client.uploadBlob(convertDataURIToUint8Array(thumbnail), {
      encoding: 'image/png',
    })
    await client.call(post, {
  ...
  <TabItem value="go" label="Go">
    Upload the thumbnail with `RepoUploadBlob`, then reference it in an `EmbedExternal` embed:

    ```go
    thumbBytes, _ := os.ReadFile("bsky_thumb.png")
    uploadResp, _ := comatproto.RepoUploadBlob(ctx, client, bytes.NewReader(thumbBytes))
        ...
        EmbedExternal: &appbsky.EmbedExternal{
          External: &appbsky.EmbedExternal_External{
            ...
            Thumb: &lexutil.LexBlob{
              Ref:      uploadResp.Blob.Ref,
              MimeType: http.DetectContentType(thumbBytes),
              Size:     uploadResp.Blob.Size,
            },
          },
        },
